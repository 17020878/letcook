$(document).ready(function () {
	...
	var bot = '<div class="chatCont" id="chatCont">' +
		...
		'<img src="/Let_Cook/chatbot-ui/Chatbot-5.jpg" class="img-circle img-profile">' +
		'</div><!--col-hgt end-->' +
		'<div class="col-hgt">' +
		'<div class="chat-txt">' +
		'' +
		...
		setTimeout(function () {
			...
			if ($.trim(val) == '' || val == 'error') { //if there is no response from bot or there is some error
				...
				for (var i = 0; i < val.length; i++) {
					if (val[i].custom) {
						var componentType = val[i].custom.accessory.type
						if (componentType === 'datepicker') {
							count++
							msg += '<div class="botResult"><div style="display: flex; flex-direction: row; justify-content: space-between;"><div>Chọn ngày rồi bấm nút xác nhận nhé</div><button type="button" class="btn btn-success btn-datepicker-confirm' +  count + '">Xác nhận <i style="color:#fff;" class="fa fa-check-circle"></i></button></div><br><input type="text" class="form-control date input-datepicker' + count + '" placeholder="Pick the multiple dates" /></div><div class="clearfix"></div>';
							useDatePicker = true
						}
					} else if (val[i]["image"]) { //check if there are any images
				...
				if (useDatePicker) {
					...
					$(document).on("click", ".btn-datepicker-confirm" + count, function () {
						var inputDate = $('.input-datepicker' + count).val();
						$('.input-datepicker' + count).prop('disabled', true);
						setUserResponse(inputDate);
						console.log('input::', inputDate)
						send(inputDate);
						$('.btn-datepicker-confirm' + count).remove();
					});
				}
			}
